(() => {
 ...
 if($$('[data-runtime-chart]').length)fetch((document.querySelector('meta[name=site-base]')?.content || '')+'/data/runtime.json').then(r=>{if(!r.ok)throw Error('data unavailable');return r.json();}).then(d=>{const rows=d.rows;const sum=names=>rows.filter(r=>names.includes(r.component)).reduce((a,r)=>a+r.layers,0);const total=rows.reduce((a,r)=>a+r.layers,0);const a=sum(['In-place multiplication']),b=sum(['Unary lookup','Phase-fix lookup','Initial lookup']),c=sum(['Square subtraction']);const vals=[a,b,c,total-a-b-c].map(v=>v*d.seconds_per_layer/3600);$$('[data-runtime-chart]').forEach(chart=>[...chart.querySelectorAll('.chart-row')].forEach((row,i)=>{row.querySelector('.bar').style.width=(vals[i]/400*100)+'%';row.lastElementChild.textContent=vals[i].toFixed(1)+' h';}));}).catch(()=>{document.querySelectorAll('[data-runtime-chart]').forEach(chart=>{const note=document.createElement('p');note.setAttribute('role','status');note.textContent='Live data could not load. The displayed values are the saved Table I calculation.';chart.after(note);});});
